...
class TestRoiTools(object):
    ...
    def test_read_mini_batches(self):
        ...
        assert batch1["gt_objects"].shape == (1,)
        np.testing.assert_equal(
            {"class": "person", "bbox": np.array([214, 121, 216, 300])},
            batch1["gt_objects"][0])
        ...
        # Checking ground truth information
        assert batch2["gt_objects"].shape == (4,)
        np.testing.assert_equal(batch2["gt_objects"][0],
            {"class": "aeroplane", "bbox": np.array([124, 166, 325, 224])})
        np.testing.assert_equal(batch2["gt_objects"][1],
            {"class": "aeroplane", "bbox": np.array([159, 187,  76,  74])})
        np.testing.assert_equal(batch2["gt_objects"][2],
            {"class": "person", "bbox": np.array([234, 384, 21, 104])})
        np.testing.assert_equal(batch2["gt_objects"][3],
